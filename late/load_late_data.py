...
import numpy as np
late_data = ad.read_h5ad("late-pathology_adata.h5ad")
from scBC.model import scBC
my_model = scBC(adata=late_data)
my_model.train_VI()
my_model.get_reconst_data(n_samples=10)
#my_model.get_edge()
edge=np.load("late_edge_arr.npy")
my_model.edge=edge
my_model.Biclustering(L=6)
with open('late_model_output.txt', 'w') as file:
    for i, entry in enumerate(my_model.S, start=1):
        ...
        gene_names = late_data.var_names
